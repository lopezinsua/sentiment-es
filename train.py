"""
sentiment-es: Fine-tuning RoBERTa-BNE para clasificacion de sentimiento en espanol.

Dataset: cardiffnlp/tweet_sentiment_multilingual (tweets reales en espanol, anotacion humana).
Modelo: PlanTL-GOB-ES/roberta-base-bne - RoBERTa entrenado en 570 GB de texto espanol (BNE).
"""
import os, json, sys, torch, numpy as np, matplotlib
matplotlib.use("Agg")
...
# Las etiquetas vienen como strings ('0','1','2') — convertir a int
...
for ex in train_data.select(range(3)):
    print(f"  [{ID2LABEL[ex['label']]}] {ex['text'][:80]}")
...
print("\nColumnas tokenizadas:", train_tok.column_names)
...
ax.set_title("Training vs Eval Loss"); ax.legend()
plt.tight_layout()
